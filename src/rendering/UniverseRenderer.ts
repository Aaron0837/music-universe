...
export class UniverseRenderer {
  ...
  private resize = () => { const width = Math.max(1, this.renderer.domElement.clientWidth), height = Math.max(1, this.renderer.domElement.clientHeight), ratio = Math.min(devicePixelRatio, this.quality === 'low' ? 1.25 : 1.8); this.renderer.setPixelRatio(ratio); this.renderer.setSize(width, height, false); this.camera.aspect = width / height; this.camera.updateProjectionMatrix(); this.instance?.resize(width, height, ratio); };
  dispose(): void { this.running = false; cancelAnimationFrame(this.frameId); this.resizeObserver.disconnect(); document.removeEventListener('visibilitychange', this.visibility); window.removeEventListener('resize', this.resize); this.interaction.dispose(); this.instance?.dispose(); this.renderer.dispose(); }
}
