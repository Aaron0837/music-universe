...
export function DeckPanel({ id }: { id: DeckId }) {
  ...
  return <article className={`deck-panel deck-panel--${id.toLowerCase()}`} data-testid={`deck-${id.toLowerCase()}`}
    ...
    <header><span>DECK {id}</span><i>{deck.status.toUpperCase()}</i><b>{(deck.bpm * Math.pow(2, deck.keyShift / 12)).toFixed(1)} BPM</b></header>
    ...
    <p className="rate-note">BPM 与 Harmony 联动音高和速度 · 暂无保调变速</p>
