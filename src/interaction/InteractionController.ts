...
export class InteractionController {
  ...
  private update(event: PointerEvent): void {
    ...
    this.ray.setFromCamera(new THREE.Vector2(this.frame.x, this.frame.y), this.camera);
    if (this.ray.ray.intersectPlane(this.plane, this.point)) { this.frame.worldX = this.point.x; this.frame.worldY = this.point.y; }
  }
  private pointerMove = ((event: PointerEvent) => { this.pointers.set(event.pointerId, event); this.update(event); if (this.frame.pressed) this.frame.energy = Math.min(1, this.frame.energy + 0.035); }) as EventListener;
  ...
  private pointerUp = ((event: PointerEvent) => { this.pointers.delete(event.pointerId); this.frame.pressed = this.pointers.size > 0; }) as EventListener;
