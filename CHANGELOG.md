...
- 数据层的单元测试（`src/data/WebLibraryRepository.test.ts`，16 个用例）：设置读写、歌单增删改查、歌词与 `hasLyrics` 同步、分析缓存与失效判定、删除的六表级联、导入失败聚合，以及 v3→v4 迁移的数据完整性。此前数据层只被端到端测试间接覆盖。

### Changed

- 本地数据库升到 Dexie v4：移除六个从未被查询的索引（`tracks` 的 `title` / `artist` / `album` / `bpm`、`playlists` 的 `name`、`analyses` 的 `version`）。曲库页的搜索、排序与筛选一直是在内存里对已加载数组做的，这些索引从第一天起就只是写入开销。**迁移不涉及数据变换——曲目、音频、封面、歌单、设置一律保留。**
- 移除 `Track.musicalKey` 与 `AnalysisRecord.musicalKey`：两处类型声明，全仓库零写入者。

### Fixed

- 批量导入半途失败会留下"看不见的曲目"。`importFiles` 此前遇到任何一首失败就抛错，调用方跳进 catch 分支、跳过刷新列表，于是已经落盘的曲目在界面上完全不可见，要切一次页面才突然出现。现在逐文件失败被收集进返回值（`ImportOutcome`），**其余文件继续导入**，调用方无论如何都刷新列表并报告聚合结果。
